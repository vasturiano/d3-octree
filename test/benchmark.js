...
for (let j = 0; j < n; ++j) {
  points1[j] = [Math.random() * 99, Math.random() * 99, Math.random() * 99];
}

for (let j = 0; j < n; ++j) {
  points2[j] = [points1[j][0] + Math.random(), points1[j][1] + Math.random(), points1[j][2] + Math.random()];
}
...
time("create", () => {
  root = octree().extent([[0, 0, 0], [100, 100, 100]]).addAll(points1);
});
